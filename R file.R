...
g4 <- ggplot(pc2, aes(x=parcel_return_case, fill=time_of_delivery)) + geom_bar(position = "stack") + theme( axis.title.y  = element_blank()) +scale_fill_manual(values = c("#63c5da","#FFD300","#FF6347"), name="Time of Delivery", labels=c("Afternoon", "Evening", "Morning")) + labs(title = "Distribution Of Parcel Lost/ Retuned to Warehouse By Time Of Delivery", x="Parcel Lost/ Retuned to Warehouse")
...
############################################
#Promo code usages 
#ggplot(parcel_without_NA, aes(x=promotions_purchase))+geom_bar()

ggplot(delivery_time_difference,aes(x =delivery_location,y=parcel_arrived_day, col = priority_delivery )) + coord_flip()+ geom_segment(aes(x=delivery_location, xend=delivery_location, y=0, yend=parcel_arrived_day))+ geom_point()+ labs(title = "Average Delivery Time According To Location And Priority Status", x= "Average Delivery Time(in days)", y="Delivery location") 
#+scale_fill_manual(values = c("#63c5da","#FF6347"), name="Priority Status", labels=c("No", "Yes")) + theme(legend.position = "bottom")

#Graph : Priority Distribution of Parcels By Parcel Status
ggplot(parcel_without_NA, aes(x=priority_delivery, fill=parcel_status)) + geom_bar(position = "stack")  + scale_fill_manual(values = c("#63c5da","#FFD300","#FF6347"), name="Parcel Status", labels=c("Delivered", "Lost", "Returned to Warehouse")) + labs(title = "Priority Distribution of Parcels By Parcel Status", x="Priority Delivery", y="Parcel Count")

#Graph : Distribution Of Parcel Returned To Warehouse By Time Of Delivery
ggplot(pc1, aes(x=parcel_return_case, fill=time_of_delivery)) + geom_bar()  +scale_fill_manual(values = c("#63c5da","#FFD300","#FF6347"), name="Time of Delivery", labels=c("Afternoon", "Evening", "Morning")) + labs(title = "Distribution Of Parcel Returned To Warehouse By Time Of Delivery", x="Parcel Retuned to Warehouse", y="Parcel Count")

#ggplot(jd1,aes(time,call_in_mins, fill = waiting_in_mins ))+ geom_tile() + scale_fill_gradient(low="white", high="blue")
